...
float4 BorderPass( float4 colorInput, float2 tex )
{
  ...
  float2 border = (pixel * border_width_variable); //Translate integer pixel width to floating point
  ...
  //colorInput.rgb = (within_border.x * within_border.y) ?  colorInput.rgb : border_color_float ; //if the pixel is within the border use the original color, if not use the border_color
