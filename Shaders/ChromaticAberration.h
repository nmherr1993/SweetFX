...
float4 ChromaticAberrationPass( float4 colorInput, float2 tex )
{
  ...
  '------------------*/
    ...
    color.r = myTex2D(s0, tex + (pixel * Chromatic_shift)).r;
    color.g = colorInput.g;
    color.b = myTex2D(s0, tex - (pixel * Chromatic_shift)).b;
  ...
  '-------------*/
  	...
  	//Set the center
		...
		//Adjust the ratio
		distance_xy *= float2((pixel.y / pixel.x),Chromatic_Ratio);
