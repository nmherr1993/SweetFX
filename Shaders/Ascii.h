...
float3 AsciiPass( float2 tex )
{
  ...
  #else //Ascii_font == 1
		float2 Ascii_font_size = float2(5.0,5.0); //5x5
  ...
  #else
    #define asciiSampler s0
  #endif

/*
	//-- Pattern 1 --
  float3 color = myTex2D(asciiSampler, cursor_position + float2(3.5,4.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2(2.5,2.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2(4.5,1.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2(5.5,3.5) * pixel).rgb;
...
/*
	...
	//-- Pattern 1b --
  float3 color = myTex2D(asciiSampler, cursor_position + float2(2.5,0.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2(0.5,4.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2(6.5,2.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2(4.5,6.5) * pixel).rgb;
...
*/
  ...
  float3 color = myTex2D(asciiSampler, cursor_position + float2( 1.5, 1.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2( 1.5, 3.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2( 1.5, 5.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, cursor_position + float2( 0.5, 6.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2( 3.5, 1.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2( 3.5, 3.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2( 3.5, 5.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, cursor_position + float2( 2.5, 6.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2( 5.5, 1.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2( 5.5, 3.5) * pixel).rgb;
  color += myTex2D(asciiSampler, cursor_position + float2( 5.5, 5.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, cursor_position + float2( 4.5, 6.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, cursor_position + float2( 6.5, 0.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, cursor_position + float2( 6.5, 2.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, cursor_position + float2( 6.5, 4.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, cursor_position + float2( 6.5, 6.5) * pixel).rgb;
...
/*	
	//-- Pattern 3 - Just one --
	float3 color = myTex2D(asciiSampler, cursor_position + float2(4.0,4.0) * pixel)	.rgb;
*/
  ...
  #if Ascii_input_image == 2
    // Linearize depth
    const float z_near = Depth_z_near;   // camera z near
    const float z_far  = Depth_z_far; // camera z far
    float luma = (2.0 * z_near) / ( -(z_far - z_near) * color.r + (z_far + z_near) );
  #else
...
/*
  ...
  //float grid_position = frac( dot(cursor_position,((screen_size / Ascii_block) * float2(0.6,0.8) )) + 0.25);
  float grid_position = frac( dot(cursor_position,((screen_size / Ascii_block) * float2(0.75,0.5) )) + 0.25);
  //float grid_position = frac( dot(cursor_position,((screen_size / Ascii_block) * float2(0.75,0.5) )) + 0.25);
...
*/  
  ...
  '---------------------------*/
		...
		*/
		...
		float n = (gray < (9./num_of_chars)) ? n12345678 : n910111213;
...
float3 AsciiWrap(float4 position : SV_Position, float2 texcoord : TEXCOORD0) : SV_Target
{  
	float3 color = AsciiPass(texcoord);
	
	#ifdef Shared_Piggyback_Ascii
